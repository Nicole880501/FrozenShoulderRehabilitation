...
try
{
	...
	if ( $query = $conn->query($sql) ) 
	{
		...
		{
			echo"<script type='text/javascript'>alert('帳號或密碼錯誤'); location='{{ url_for('login') }}';</script>";
		}
		else
		{	
			$_SESSION['i_id'] = $i_id;
			echo"<script type='text/javascript'>alert('歡迎進入居家自主復健系統');location='{{ url_for('training') }}';</script>";
		}
	}
	else
	{
